...
class CitationCheckRepairTests(unittest.TestCase):
    def run_stage0a(self, paper_dir: Path):
        subprocess.run(
            [sys.executable, str(ROOT / "skill/scripts/stage0a_parse.py"), str(paper_dir)],
            check=True,
    ...
    def run_track_a(self, paper_dir: Path):
        subprocess.run(
            [sys.executable, str(ROOT / "skill/scripts/track_a_check.py"), str(paper_dir)],
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
...
""",
    ...
    def test_a4_reports_key_missing_from_evidence_pack(self):
        track_a = load_module("track_a_check", ROOT / "skill/scripts/track_a_check.py")
    ...
    def test_stage0b_only_preserves_existing_evidence_pack_entries(self):
        stage0b = load_module("stage0b_fetch_evidence", ROOT / "skill/scripts/stage0b_fetch_evidence.py")
        with tempfile.TemporaryDirectory() as td:
            paper_dir = Path(td)
